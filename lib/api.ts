...
export async function fetchProducts(page = 1, limit = 12, category = "") {
  ...
  const res = await fetch(url, { next: { revalidate: 60 } }); // Cache for 60 seconds (ISR)
...
export async function fetchProduct(id: string) {
  const res = await fetch(`${API_URL}/products/${id}`, { next: { revalidate: 60 } }); // Cache for 60 seconds (ISR)
